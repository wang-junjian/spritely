 ...
 */
function normalizeBackground(raw: unknown): BackgroundState | null {
  ...
  if (typeof raw === 'string') {
    ...
    if (raw.includes('gradient')) return { kind: 'gradient', value: raw, fit: 'contain', veil: DEFAULT_VEIL }
    return { kind: 'color', value: raw, fit: 'contain', veil: DEFAULT_VEIL }
  }
  if (typeof raw === 'object') {
    const candidate = raw as Partial<BackgroundState>
 ...
 * @returns the source.
 */
export function createBackgroundSource(): BackgroundSource {
  const store = createSnapshotStore<BackgroundState | null>(null, { persist: { name: BACKGROUND_PERSIST_KEY } })
 ...
 */
function toImageCss(state: BackgroundState): string {
...
export class BackgroundPresenter {
  ...
  private state: BackgroundState | null = null
